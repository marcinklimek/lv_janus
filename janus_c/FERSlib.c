...
int FERS_WriteBoardInfo(int handle, FERS_BoardInfo_t binfo)
{
...
#ifdef FERS_5202
	const int CalibPage = FLASH_PEDCALIB_PAGE;
#elif FERS_5203
	const int CalibPage = FLASH_THR_OFFS_PAGE;
#endif
	...
	memcpy(bic + 44, &CalibPage, 2);			// Calibration page	    (2) (pedestal in A5202, threshold offset in A5203)
...
// Return:		0=OK, negative number = error code
// --------------------------------------------------------------------------------------------------------- 
int FERS_CheckBootloaderVersion(int handle, int* isInBootloader, uint32_t* version, uint32_t* release)  // It is called only for eth and usb
{
	char buffer[32];
	uint32_t* p_intdata;
	int res;
...
int FUP_CheckControllerVersion(int handle, int* isValid, uint32_t* version, uint32_t* release) // Called if TDL connection
{
	*isValid = 0;
	*version = 0;
	*release = 0;
...
int FERS_FirmwareUpgrade(int handle, FILE* fp, void(*ptr)(char* msg, int progress))
{
	...
	if (FERS_CONNECTIONTYPE(handle) == FERS_CONNECTIONTYPE_TDL)
		FUP_WriteFPGAFirmwareOnFlash(handle, firmware, firmware_size_byte, ptr);
	else
