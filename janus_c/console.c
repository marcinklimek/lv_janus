...
void* ListenThread(void *arg) {
    ...
    // Receive until the peer closes the connection or an error coour in the socket
    do {
        ...
        if (size > 0) {
            ...
            Con_printf("L", FONT_STYLE_BOLD COLOR_RED "ERROR: recv failed with error: %d\n" COLOR_RESET, f_socket_errno); // WSAGetLastError());
        }
    } while (size > 0);
    lock(Sbuff.mutex);
...
int ConnectSocket()
{
    ...
    if (WSAStartup(MAKEWORD(2, 2), &wsa) != 0) {	// not needed for linux??
        Con_printf("L", FONT_STYLE_BOLD COLOR_RED "ERROR: Socket init failed. Error Code : %d" COLOR_RESET, f_socket_errno); // WSAGetLastError());
        return -1;
    }
#endif
    ...
    if ( iResult != 0 ) {
        Con_printf("L", FONT_STYLE_BOLD COLOR_RED "ERROR: socket getaddrinfo error: %d" COLOR_RESET, iResult);
#ifdef _WIN32
        WSACleanup();
#endif
        return -11;
    ...
    for (ptr = result; ptr != NULL; ptr = ptr->ai_next) {
        ...
        if (ConnectSocket == f_socket_invalid) {
            Con_printf("L", FONT_STYLE_BOLD COLOR_RED "ERROR: socket connect failed (error: %ld)\n" COLOR_RESET, f_socket_errno); // WSAGetLastError());	// only windows
#ifdef _WIN32
            WSACleanup();
#endif
            return -1;
    ...
    freeaddrinfo(result);
    if (ConnectSocket == f_socket_invalid) {
        Con_printf("L", FONT_STYLE_BOLD COLOR_RED "ERROR: Unable to connect to server!\n" COLOR_RESET);
#ifdef _WIN32
        WSACleanup();
#endif
        return -1;
...
int SendDataToGUI(char* data, int size)
{
    ...
    if (send(ConSocket, data, size, 0) < 0) {	// send IS multiplatform!!!
        Con_printf("L", FONT_STYLE_BOLD COLOR_RED "ERROR: send data to socket failed\n" COLOR_RESET);
#ifdef _WIN32
